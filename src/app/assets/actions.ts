...
async function findOrCreateInstrument(
  ...
  const { data: created, error } = await admin
    .from("market_instruments")
    .insert({ asset_class_id: assetClassId, ticker, name })
    .select("id")
    .single();
...
async function buildPayload(
  ...
  if (slug === "bitcoin") {
    const quantity = requiredDecimal(formData, "quantity", "Quantidade de BTC");
    if (quantity <= 0) throw new Error("Quantidade deve ser maior que zero.");
    const averagePrice = parseDecimal(String(formData.get("average_price") ?? ""));
    const local = String(formData.get("local") ?? "").trim();
    const instrumentId = await findOrCreateInstrument(assetClass.id, "BTC", "Bitcoin");
    return {
      name: "Bitcoin",
      quantity,
      average_price: averagePrice,
